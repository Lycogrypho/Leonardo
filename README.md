...
- **Rich AST Representation**: Expressions are represented as a type-safe AST with nodes for:
  ...
  - Power operations
  - Higher-order operators (derivatives, definite integrals via Simpson's rule, indefinite integrals via a symbolic rule table with integration by parts, trig-power reduction formulas (including `tan`/`sec`/`csc`/`cot` powers), full rational-function partial fractions (repeated and complex roots at any degree, via square-free factorisation), **non-linear u-substitution** for `f(g(x))·g'(x)` forms — `∫ x·e^(x²)`, `∫ sin³x·cos x` — that the linear chain rule cannot reach, **trigonometric/hyperbolic substitution** for radicals `√(a²−x²)` / `√(a²+x²)` / `√(x²−a²)`, the **Weierstrass half-angle substitution** `t = tan(x/2)` for rational functions of `sin`/`cos` (`∫ 1/(1+sin x)`, `∫ 1/(2+cos x)`), the **special integral functions** `Si`/`Ci`/`Ei`/`li`/`fresnelS`/`fresnelC` as named answers to the classic non-elementary integrals (`∫ sin x/x = Si(x)`, `∫ e^(−x²) = (√π/2)·erf(x)`), and a data-driven table covering `tan`, `cot`, `sec`, `csc`, the hyperbolics, `a^v`, `ln(v)^2` and the cyclic `e^v sin(v)` / `e^v cos(v)` — table rules are **parameterised**, so a symbolic constant free of the integration variable is handled too (`k^v` → `k^v/ln(k)`, `1/(a²+v²)` → `atan(v/a)/a`), and every entry carries the linear-argument chain rule, so `tan(2x+1)` closes too, and limits)
...
- Broader indefinite integration (bulk table transcription from external references)
