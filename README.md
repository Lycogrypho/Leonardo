...
- **Rich AST Representation**: Expressions are represented as a type-safe AST with nodes for:
  ...
  - Power operations
  - Higher-order operators (derivatives, definite integrals via Simpson's rule, indefinite integrals via a symbolic rule table with integration by parts, trig-power reduction formulas, rational-function partial fractions, **non-linear u-substitution** for `f(g(x))·g'(x)` forms — `∫ x·e^(x²)`, `∫ sin³x·cos x`, `∫ x·√(1+x²)` — that the linear chain rule cannot reach, and a data-driven table covering `tan`, `cot`, `sec`, `csc` and their squares, the hyperbolics, `a^v`, `ln(v)^2` and the cyclic `e^v sin(v)` / `e^v cos(v)` — table rules are **parameterised**, so a symbolic constant free of the integration variable is handled too (`k^v` → `k^v/ln(k)`, `1/(a²+v²)` → `atan(v/a)/a`), and every entry carries the linear-argument chain rule, so `tan(2x+1)` closes too, and limits)
...
- Broader indefinite integration (trig/hyperbolic-substitution; repeated/complex roots at degree ≥ 3 for rational functions)
