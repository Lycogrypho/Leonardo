...
final class Session:
  ...
  private var colorSchemeName: String = ColorSchemes.Default
  ...
  private def serializeValue(v: _Value): String = v match
    ...
    // _Number.render, not d.toString: a bare Double renders the JavaScript way under Scala.js
    // ("3" for "3.0"), and this is the `:save` path -- so a session saved in a browser would
    // not be byte-identical to the same session saved on the JVM (issue F_0003).
    case _Number(d) => _Number.render(d)
  ...
  private def setColors(name: String): String =
    if ColorSchemes.isValid(name) then
      colorSchemeName = name
      s"colors = $name"
    else
      s"unknown color scheme '$name'; available: ${ColorSchemes.listed}"
...
object Session:
   ...
   *
   *  **The storage medium is platform-specific; this dispatch is not** (issue F_0003 phase 2).
   *  `SessionIO` is a file path on the JVM and a `localStorage` key in the browser, so
   *  `:load work` means the same thing in both and only the medium differs.  What made that
   *  cheap is that `Session.load` was already pure string-in: the file system was the only
   *  JVM-bound part of the command, and it was never inside `Session`.
   *
   *  @param session the session to replay the script into
   *  @param path    the session name — a file path on the JVM, a storage key in a browser
   *  @return the script's own output, or a message explaining why it could not be read
   */
  def loadFile(session: Session, path: String): String =
    SessionIO.read(path) match
      case Right(text) => session.load(text)
      case Left(why)   => s"could not read $path: $why"

  /** Writes the session's replayable script.  Backs `:save`; see [[loadFile]] on storage.
   *
   *  @param session the session to serialise
   *  @param path    the session name — a file path on the JVM, a storage key in a browser
   *  @return a confirmation, or a message explaining why it could not be written
   */
  def saveFile(session: Session, path: String): String =
    SessionIO.write(path, session.script) match
      case Right(_)  => s"saved to $path"
      case Left(why) => s"could not write $path: $why"
