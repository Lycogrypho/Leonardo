...
- **Rich AST Representation**: Expressions are represented as a type-safe AST with nodes for:
  ...
  - Power operations
  - Higher-order operators (derivatives, definite integrals via Simpson's rule, indefinite integrals via a symbolic rule table with integration by parts, and limits)
...
- Broader indefinite integration (partial fractions for rational functions, non-linear substitution, reduction formulas)
